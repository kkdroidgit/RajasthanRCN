...
contract Complain {
  ...
  enum Status { Pending, Accepted, Resolved, Proposed }
    ...
    mapping(uint => Complain_Reg) private complainMap;
    ...
    //View complains "complainMap[cid]._reward"
    function viewComplain(uint cid) public constant returns (uint, uint, uint, uint, string, Status) {
        return (cid, complainMap[cid]._long, complainMap[cid]._lat, complainMap[cid]._category, complainMap[cid]._data, complainMap[cid]._status);
    }
    ...
    function changeStatus(uint cid, Status sta) public policeAccess {
        ...
        //if someone else solved then police update to resolved
  ...
  /*  function resolve(uint cid) public complainOwnerAccess(cid) {
    ...
    //to be used in UI for entering the proposal
    ...
    //fund complain will be used latter on
